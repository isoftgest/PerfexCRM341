...
/**
 * Controlador puente para exponer /admin/rental_properties sin depender del cargador de rutas del módulo.
 * Mantiene la lógica dentro de modelos/vistas del módulo Rentals y evita tocar controladores core existentes.
 */
class Rental_properties extends AdminController
{
    protected $permission = 'rentals_properties';
    protected $model_name = 'Rental_properties_model';
    protected $view_folder = 'properties';
    protected $lang_key = 'rental_properties';
    protected $route = 'rental_properties';
    ...
    public function __construct()
    {
        ...
        if (!is_admin() && !has_permission($this->permission, '', 'view')) { access_denied($this->permission); }
    }

    public function index()
    {
        $data['title'] = _l($this->lang_key);
        $data['items'] = $this->{$this->model_name}->get();
    ...
    public function form($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url($this->route));
        }
        $data['item'] = $id === '' ? null : $this->{$this->model_name}->get($id);
        $data['title'] = $id === '' ? _l('new_record') : _l('edit');
        $this->load->view('rentals/' . $this->view_folder . '/property', $data);
    }
